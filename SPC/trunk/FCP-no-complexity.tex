...
The $\mathsf{FCP}$ version of the problem is to find the fewest number of triangles that we must give in order for there to be a unique triangle partition. For example, note that in Figure~\ref{fig:holyergraph}, once you specify one triangle in the partition, there is only one way to partition the graph in accordance with that triangle.
...
%
% 
Holyer uses the graph in Figure~\ref{fig:holyergraph} to represent variables and literals. Since each graph can only be partitioned in one of two ways, one way will correspond to a true setting, the other will correspond to a false setting. He calls these the $T$-partition and the $F$-partition. 
